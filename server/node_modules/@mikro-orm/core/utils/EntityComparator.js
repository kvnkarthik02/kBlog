...
class EntityComparator {
    ...
    getPkGetterConverted(meta) {
        ...
        if (meta.primaryKeys.length > 1) {
            ...
            if (meta.properties[pk].customType) {
                context.set(`convertToDatabaseValue_${pk}`, (val) => meta.properties[pk].customType.convertToDatabaseValue(val, this.platform, { mode: 'serialization' }));
    ...
    getEmbeddedPropertySnapshot(meta, prop, context, level, path, dataKey, object = prop.object) {
        ...
        ret += meta.props.filter(p => p.embedded?.[0] === prop.name).map(childProp => {
            ...
            if (shouldProcessCustomType(childProp)) {
                context.set(`convertToDatabaseValue_${childProp.name}`, (val) => childProp.customType.convertToDatabaseValue(val, this.platform, { mode: 'serialization' }));
    ...
    getPropertySnapshot(meta, prop, context, dataKey, entityKey, path, level = 1, object) {
        ...
        if (prop.reference === enums_1.ReferenceType.ONE_TO_ONE || prop.reference === enums_1.ReferenceType.MANY_TO_ONE) {
            ...
            if (prop.customType) {
                context.set(`convertToDatabaseValue_${prop.name}`, (val) => prop.customType.convertToDatabaseValue(val, this.platform, { mode: 'serialization' }));
